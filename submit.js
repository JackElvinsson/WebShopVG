...
function displayConfirmation(confirmation) {
    ...
    // Create container for order details
    ...
    for (const id in cart) {
        ...
        itemsHtml += `
        <li>${item.title}: $${item.price} x ${item.quantity} = $${item.price * item.quantity}</li>
      `;
    }
    ...
    orderDetails.innerHTML = `
      ...
      <ul>
        ...
        ${itemsHtml}
        <br>
        <li>Totalt pris: $${totalPrice.toFixed(2)}</li>
      </ul>
    `;

    // Add order details to order container
